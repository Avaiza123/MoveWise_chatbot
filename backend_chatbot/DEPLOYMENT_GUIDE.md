...
This project is ready to run as a Hugging Face Docker Space.

### Step 1: Prepare the Space

1. Go to Hugging Face and create a new Space.
2. Set the owner to your account or organization.
3. Set the Space name to `MoveWise_chatbot`.
4. Choose a short description such as `Fitness chatbot API for Flutter apps`.
5. Select the `Docker` SDK.
6. Choose the `Docker` template.
7. Set visibility to `Public` unless you need the code private.

### Step 2: Add the Dockerfile

Place the root-level [Dockerfile](Dockerfile) in this repository so the Space can build the Flask backend from `backend_chatbot/`.

### Step 3: Push the code

1. Commit and push the repository to the Hugging Face Space Git remote.
2. Wait for the build to complete.
3. Open the Space URL and verify `GET /health`.

### Step 4: Flutter integration

Use the Space URL as the base API URL in Flutter, for example:

```dart
https://your-space-name.hf.space/api/chat
```

### Step 5: What to check if the build fails

1. Confirm `backend_chatbot/requirements.txt` installs successfully.
2. Confirm the container starts with `gunicorn` on port `7860`.
3. Confirm the Flask app is imported as `app:app` from `backend_chatbot/app.py`.
...
[Service]
User=ubuntu
WorkingDirectory=/home/ubuntu/fitness-chatbot/backend_chatbot
Environment="PATH=/home/ubuntu/fitness-chatbot/backend_chatbot/venv/bin"
ExecStart=/home/ubuntu/fitness-chatbot/backend_chatbot/venv/bin/gunicorn \
    --workers 4 \
    --bind 127.0.0.1:5000 \
    --timeout 120 \
    app:app
